...
public class TransactionService {
    ...
    public TransferRespDto 이체하기(TransferReqDto transferReqDto, Long withdrawNumber, Long depositNumber,
        ...
        // 구분값 검증
        // 출금계좌 존재확인
        if (TransactionEnum.valueOf(transferReqDto.getGubun()) != TransactionEnum.DEPOSIT) {
    ...
    public DepositRespDto 입금하기(DepositReqDto depositReqDto) {
        ...
        Account depositAccountPS = accountRepository.findByNumber(depositReqDto.getNumber())
                .orElseThrow(() -> new CustomApiException("입금계좌 검증 실패", HttpStatus.BAD_REQUEST));
        ...
        // 실행 - 거래내역 insert, 잔액 update
        depositAccountPS.입금하기(depositReqDto.getAmount()); // 계좌잔액 update, 더티체킹
        Transaction transaction = depositReqDto.toEntity(depositAccountPS); // history save
        Transaction transactionPS = transactionRepository.save(transaction);
        // toEntity 만들어서 transaction에 save 하거나
        // transaction entity에 입금하기 메서드 만들어주거나
